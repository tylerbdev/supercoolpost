			...
			//Pagination	
				...
				for($count = $posts_page_start; $count < $posts_page_start+1 && $count < count($blogs_row); $count++){
					if($blogs_row[$count]['draft'] == 0){
						echo '<span class="post_listing"><a href="index.php?p=' . $blogs_row[$count]['id'] . '"><span class="post_listing_title">' . $blogs_row[$count]['title'] . '</span></a></br><span class="datetime">' . $blogs_row[$count]['datetime'] . '</span><p><span class="post_listing_body">' . $blogs_row[$count]['body'] . '</span></p></span>';			
					}
				}
			}
		//Individual post
				if(isset($_GET['p'])){
					...
					echo '<p class="post_body">' . $post_row[0]['body'] . '</p>';
				}
